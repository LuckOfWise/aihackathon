...
metadata:
    github-path: skills/claude-code-review
    github-ref: refs/heads/main
    github-repo: https://github.com/aki77/claude-code-review
    github-tree-sha: 6bfa5981775772cdc7ea44136cde4e7df9702905
...
1. **CLI を `--crit` 付きで実行し、出力を `crit comment --json` にパイプで渡す**。
...
2. **crit のレビュー画面を開く**。crit コマンドを直接叩くのではなく、**`crit` スキル
...
3. このスキルの役割は「レビュー結果を crit に流し込んで画面を開くまで」。crit 上での
...
- **カレントディレクトリがレビュー対象のリポジトリルートであること。** そうでなければ
  移動してから実行する。
- **引数の解釈**: スキルに渡された引数のうち `--fix` / `--crit` を除いた最初の非フラグ語が
...
- **exit 1**: confirmed 指摘が 1 件以上ある（`local` の既定挙動）。**唯一「1」を返す
