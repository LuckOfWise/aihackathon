...
- オブジェクトの取得は**ログインユーザーを起点に行う**。PunditやCanCanCanなどのライブラリを導入し、権限チェック漏れを自動で検知する仕組みを採用する。
...
### ビューのシンプル化
- ビューには複雑なロジックを書かず、**ActionViewヘルパー、Model、Decorator、など、適切な場所へロジックを移動させる。
- パーシャルのルート要素は**一つに限定**する。
- パーシャルのルート要素にマージンを持たせず、**親コンポーネント側でマージンを設定する**。
- 「詳細」リンクのような不要なDOMを追加する代わりに、**対象のタイトルやコンテナ要素全体を直接リンクにする**。
...
- デザインに依存しないテストを書くために、CSSクラスではなく`data-test-id`のようなテスト専用の属性を使用する。
