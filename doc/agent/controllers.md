...
```ruby
# AI: 認証が必要なコントローラーでは明示的に記述
class SomeController < ApplicationController
  before_action :authenticate_user!
end

# AI: 管理者のみアクセス可能なコントローラー
...
```ruby
# AI: シンプルにfindを使用し、存在しない場合は自動的に404が返される
def set_project
...
```ruby
# AI: 決済失敗時など、ビジネス上の理由で特別な処理が必要な場合
def create_payment
...
```ruby
# AI: strict_loadingでN+1を検出し、preloadで対策
def index
  # 開発環境でN+1を検出
  @projects = Project.strict_loading.page(params[:page])
  # N+1が検出されたら、必要な関連のみpreload
  @projects = Project.strict_loading.preload(:user, :rewards).page(params[:page])
end

# AI: joinsとpreloadを組み合わせる場合
def featured
...
```ruby
# AI: コントローラーは通常のHTMLレスポンスを返すだけ
def search
  @projects = Project.search(params[:query])
  # turbo_frameタグで囲まれた部分のみが更新される
end
```

#### turbo_driveを明示的に有効化する場合
```erb
<%# AI: 特定のリンクでturbo_driveを有効化 %>
<%= link_to "次のページ", next_page_path, data: { turbo: "true" } %>

<%# AI: フォーム送信でturbo_driveを有効化 %>
